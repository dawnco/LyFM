...
class LibFile
{
    ...
    public function unzip_file($path, $unzip_path)
    {
        ...
        if (strtolower(pathinfo($path, PATHINFO_EXTENSION)) != 'zip') {
            return array('status' => false, 'error' => 'zip路径有误');
        }
        if (!is_file($path)) {
            return array('status' => false, 'error' => 'zip文件不存在');
        }
        if (!is_dir($unzip_path)) {
            return array('status' => false, 'error' => '解压目录不存在');
        }
        $zip = new ZipArchive();
        if ($zip->open($path) !== true) {
            return array('status' => false, 'error' => '打开压缩文件失败');
        }
    ...
    private function do_path($path)
    {
        ...
        if (substr($path, 0, 1) == "/" && strpos($path, get_config('rootDir')) !== 0) {
            $path = get_config('rootDir');
        }
